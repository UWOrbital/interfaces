...
// CMD_CAPTURE_IMAGE
static void unpackCaptureImageCmdData(const uint8_t* buffer, uint32_t* offset, cmd_msg_t* msg);
...
static const unpack_func_t unpackFns[] = {
    ...
    [CMD_CAPTURE_IMAGE] = unpackCaptureImageCmdData,
...
// CMD_CAPTURE_IMAGE
static void unpackCaptureImageCmdData(const uint8_t* buffer, uint32_t* offset, cmd_msg_t* cmdMsg) {
  cmdMsg->captureImage.cameraId = unpackUint8(buffer, offset);
}
