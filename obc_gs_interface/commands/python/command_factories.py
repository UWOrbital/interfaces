...
def create_cmd_cc1120_register_read(unixtime_of_execution: int | None = None) -> CmdMsg:
    """
    Function to create a CmdMsg structure for CMD_CC1120_REGISTER_READ

    :param unixtime_of_execution: A time of when to execute a certain event,
                                  by default, it is set to None (i.e. a specific
                                  time is not needed)
    :return: CmdMsg structure for CMD_CC1120_REGISTER_READ
    """

    cmd_msg = CmdMsg(unixtime_of_execution)
    cmd_msg.id = CmdCallbackId.CMD_CC1120_REGISTER_READ
    return cmd_msg
...
COMMAND_FACTORIES: list[Callable[..., CmdMsg]] = [
    ...
    create_cmd_cc1120_register_read
]
